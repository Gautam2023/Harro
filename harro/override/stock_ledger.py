...
def get_segregated_bundle_entries(sle, bundle_details, batch_balance_dict, filters):
	...
	# Pre-fetch rack and bin_location from Batch for outward entries (for segregate mode)
	batch_rack_bin_map = _get_batch_rack_bin_map(
		[row.batch_no for row in bundle_details if row.batch_no and row.qty < 0]
	)
	...
	for row in bundle_details:
		...
		# Harro: For outward entries, add rack and bin_location from Batch
		if filters.get("segregate_serial_batch_bundle") and row.batch_no and row.qty < 0:
			rack_bin = batch_rack_bin_map.get(row.batch_no)
			if rack_bin:
				new_sle["rack"] = rack_bin.get("rack")
				new_sle["bin_location"] = rack_bin.get("bin_location")
...
def _get_batch_rack_bin_map(batch_nos):
	"""Fetch rack and bin_location from Batch doctype for given batch numbers."""
	if not batch_nos:
		return {}
	batch_nos = list(set(b for b in batch_nos if b))
	if not batch_nos:
		return {}
	batches = frappe.get_all(
		"Batch",
		filters={"name": ("in", batch_nos)},
		fields=["name", "rack", "bin_location"],
	)
	return {b.name: {"rack": b.rack, "bin_location": b.bin_location} for b in batches}
...
def get_columns(filters):
	...
	# Harro: Add Rack and Bin Location columns when Segregate Serial/Batch Bundle is checked
	if filters.get("segregate_serial_batch_bundle"):
		columns.extend(
			[
				{
